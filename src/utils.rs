...
/// Split a content string into individual lines for the editor.
/// Guarantees at least one (possibly empty) line is returned.
...
"#;
    ...
    /// Simulate the editor's get_content() (export): extract line strings,
    /// then call the shared lines_to_content serializer.
    fn fake_get_content(editor: &FakeTextBuffer) -> String {
        let strs: Vec<&str> = editor.lines.iter().map(|l| l.instr_text.as_str()).collect();
        lines_to_content(&strs)
    }

    /// Simulate the editor's load_content() (import): call the shared
    /// content_to_lines parser, then push each line into a FakeTextBuffer.
    fn fake_load_content(content: &str) -> FakeTextBuffer {
        let mut editor = FakeTextBuffer::default();
        for line in content_to_lines(content) {
    ...
    fn test_content_to_lines() {
        ...
        // Empty string produces one empty line (the load_content guarantee)
        assert_eq!(content_to_lines(""), vec![""]);

        // Multiple trailing newlines: only the final one is consumed by .lines()
        assert_eq!(content_to_lines("(func)\n\n"), vec!["(func)", ""]);
    }
    ...
    fn test_content_round_trip() {
        // content_to_lines → lines_to_content → content_to_lines is stable
        for input in [
            "(func\ndrop\n)",
            "(func)",
            "",
            ...
            let lines = content_to_lines(input);
            let serialized = lines_to_content(&lines);
            let re_parsed = content_to_lines(&serialized);
            assert_eq!(lines, re_parsed, "round-trip failed for input: {input:?}");
        }
    }

    #[test]
    fn test_import_export() -> Result<()> {
        // Importing a simple program produces the same result as manually pushing lines
        {
            let mut imported = fake_load_content("(func)\n");
            let mut manual = FakeTextBuffer::default();
            manual.push_line("(func)");

            assert_eq!(
                test_editor_flow(&mut manual)?,
                test_editor_flow(&mut imported)?
            );
        }

        // Importing a multi-line program matches manual line-by-line entry
        {
            let mut imported = fake_load_content("(func\ni32.const 42\ndrop\n)");

            let mut manual = FakeTextBuffer::default();
            manual.push_line("(func");
            manual.push_line("i32.const 42");
            manual.push_line("drop");
            manual.push_line(")");
        ...
        // Export then re-import round-trip compiles identically
        {
            let mut editor = FakeTextBuffer::default();
            editor.push_line("(func");
            editor.push_line("i64.const 17");
            ...
            let exported = fake_get_content(&editor);
            let mut reimported = fake_load_content(&exported);

            assert_eq!(
                test_editor_flow(&mut editor)?,
                test_editor_flow(&mut reimported)?
            );
        }

        // Round-trip preserves behavior for program with syntax errors
        {
            let mut editor = FakeTextBuffer::default();
            editor.push_line("(func");
            editor.push_line("block");
        ...
        // Import of empty content matches pushing a single empty line
        {
            let mut imported = fake_load_content("");
            let mut manual = FakeTextBuffer::default();
            manual.push_line("");
        ...
        // Round-trip of program with validation errors (impossible branch)
        {
            let mut editor = FakeTextBuffer::default();
            editor.push_line("(func");
            editor.push_line("br 144");
        ...
        // Import of content with memory declaration matches manual entry
        {
            let mut imported = fake_load_content("(memory 0)\n(func\ni32.load 144\ndrop\n)");

            let mut manual = FakeTextBuffer::default();
            manual.push_line("(memory 0)");
            manual.push_line("(func");
            manual.push_line("i32.load 144");
            manual.push_line("drop");
            manual.push_line(")");
        ...
        // Round-trip of program with inline import
        {
            let mut editor = FakeTextBuffer::default();
            editor.push_line("(func");
            editor.push_line("(import \"hello\" \"goodbye\")");
